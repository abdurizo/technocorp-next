...
function SiteView() {
    ...
    const siteViewHandler = (id) => {
        document.getElementsByTagName("html")[0].style.filter = `${id === 1 ? "none" : id === 2 ? "grayscale(100%)" : "grayscale(100%) invert(100%)"}`;
    };
    return (
        <div className={`${styles.site_view_block} site_view_block`}>
            <Image src={GlassesIcon}
                width={'auto'}
                height={'auto'}
                alt="icon glasses"
                onClick={addFrHandler}
            />
            <div className={`${styles.additional_features}  ${addFr ? styles.d_block : styles.d_none} ${addFr}`}>
                <p>Sayt ko'rinishi</p>
